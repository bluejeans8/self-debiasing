...
if __name__ == '__main__':
    ...
    for model_idx, model_name in enumerate(args.models):
        ...
        for mode in args.modes:
            ...
            input_path = os.path.join('./debiasing_ex/', f'prompted_generations_{model_name}_{mode}.txt')
            with open(input_path, 'r') as rf:
                for output, prompt in zip(rf.readlines(), prompt_iterator):
                    time.sleep(1)
                    output_text = output[1:-1]
            ...
            #         )
                    ...
                    result_scores = dict()
                    # scores = dict(json.dumps(response, indent=2))
                    scores = response
                    for key, value in scores['attributeScores'].items():
                        attr = key
                        scr = value['spanScores'][0]['score']['value']
                        result_scores[attr] = scr

                    generated_example = ModelOutput(text=output_text, scores=result_scores)
