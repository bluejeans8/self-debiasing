...
from transformers import T5Tokenizer, T5ForConditionalGeneration, GPT2Tokenizer, PreTrainedTokenizer, PreTrainedModel, \
                        LlamaTokenizer, LlamaForCausalLM, AutoTokenizer, AutoModelForCausalLM

from generation import SelfDebiasingGPT2LMHeadModel, SelfDebiasingLLamaLMHeadModel, SelfDebiasingAlpacaLMHeadModel
...
class AlpacaWrapper(ModelWrapper):

    def __init__(self, model_name: str = "/home/yena.seo/ai_projects/models/alpaca-7b", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained llama model (default: "gpt2-xl")
        :param use_cuda: whether to use CUDA
        """
        ...
        self._model = SelfDebiasingAlpacaLMHeadModel.from_pretrained(model_name, device_map="auto")
        # if use_cuda:
        #     self._model.parallelize()
        self._tokenizer.pad_token = self._tokenizer.eos_token
        self._model.config.pad_token_id = self._tokenizer.eos_token_id
